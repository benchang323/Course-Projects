	...
	while(!game_over) {
		...
		if (choice.length() != 1) {
			...
			case 'L': case 'l': {
				...
				// TODO: add try catch blocks to handle any file i/o and gmae loading problem exit the program with return code -1 if an exception is caught here
				try {
					std::string argument;
					std::cin >> argument;
					std::ifstream ifs;
